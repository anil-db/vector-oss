...
use super::{
    service::{MyVectorResponse, MyVectorService},
    sink::MyVectorSink,
    MyVectorSinkError,
};
use crate::{
...
/// Configuration for the `my_vector` sink.
#[configurable_component(sink("my_vector", "Relay observability data to a Vector instance."))]
#[derive(Clone, Debug)]
#[serde(deny_unknown_fields)]
pub struct MyVectorConfig {
    ...
    version: Option<super::MyVectorConfigVersion>,
    ...
    pub(in crate::sinks::my_vector) acknowledgements: AcknowledgementsConfig,
}

impl MyVectorConfig {
    /// Creates a `MyVectorConfig` with the given address.
...
impl GenerateConfig for MyVectorConfig {
    fn generate_config() -> toml::Value {
        toml::Value::try_from(default_config("127.0.0.1:6000")).unwrap()
    }
}

fn default_config(address: &str) -> MyVectorConfig {
    MyVectorConfig {
        version: None,
...
#[typetag::serde(name = "my_vector")]
impl SinkConfig for MyVectorConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(VectorSinkType, Healthcheck)> {
        ...
        let healthcheck_client = MyVectorService::new(client.clone(), healthcheck_uri, false);
        let healthcheck = healthcheck(healthcheck_client, cx.healthcheck);
        let service = MyVectorService::new(client, uri, self.compression);
        let request_settings = self.request.into_settings();
        let batch_settings = self.batch.into_batcher_settings()?;

        let service = ServiceBuilder::new()
            .settings(request_settings, MyVectorGrpcRetryLogic)
            .service(service);

        let sink = MyVectorSink {
            batch_settings,
            service,
        };
...
async fn healthcheck(
    ...
    match request.await {
        Ok(response) => match proto::ServingStatus::try_from(response.into_inner().status) {
            ...
            Ok(status) => Err(Box::new(MyVectorSinkError::Health {
                status: Some(status.as_str_name()),
            })),
            Err(_) => Err(Box::new(MyVectorSinkError::Health { status: None })),
        },
        Err(source) => Err(Box::new(MyVectorSinkError::Request { source })),
    }
}
...
#[derive(Debug, Clone)]
struct MyVectorGrpcRetryLogic;

impl RetryLogic for MyVectorGrpcRetryLogic {
    type Error = MyVectorSinkError;
    type Response = MyVectorResponse;

    fn is_retriable_error(&self, err: &Self::Error) -> bool {
        use tonic::Code::*;

        match err {
            MyVectorSinkError::Request { source } => !matches!(
                source.code(),
                // List taken from
                //
