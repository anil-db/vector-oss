...
pub use blocking_layer::{BlockingLayer, BlockingService};
pub use config::MyVectorConfig;

/// Marker type for the version two of the configuration for the `my_vector` sink.
#[configurable_component]
#[derive(Clone, Debug)]
enum MyVectorConfigVersion {
...
mod test {
    #[test]
    fn generate_config() {
        crate::test_util::test_generate_config::<super::MyVectorConfig>();
    }
}
...
mod tests {
    ...
    #[test]
    fn generate_config() {
        crate::test_util::test_generate_config::<MyVectorConfig>();
    }
    ...
    async fn run_sink_test(test_type: TestType) {
        ...
        let config: MyVectorConfig = toml::from_str(&config).unwrap();
